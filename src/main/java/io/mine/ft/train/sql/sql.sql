...
create table t_interact(   //Interact
   ...
   student_name           varchar2(10),--学生名称							5
   teacher_name         varchar2(10),--老师名称
   flag_answer         char(1),--是否回复
   answer_content        varchar2(2000),--回复内容
   flag_estimate       char(1),--是否评价
   estimate_content    varchar2(2000),--评价内容						10
   tuid                 integer,--老师id
   sid <!--  -->      integer,--学生id
   score                number(5,2),--评分
   create_date          date,--互动创建时间
   answer_time         date,--回复时间											15
...
create table t_accusation (     
   ...
   teacher_accusation_content varchar2(2000),--老师举报内容
   student_accusation_time date,--学生举报时间
   teacher_accusation_time date,--老师举报时间
   i_id  integer  references t_interact(INTERACT_ID) unique,--举报的互动id
   student_accusation_content varchar2(2000)--学生举报内容
);
...
create table t_user(
	...
	realname varchar2(10),--真名
	department varchar2(50),--院系
	role varchar2(10) check (role in('student','teacher'))--学生/教师
);
